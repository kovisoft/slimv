...
let g:slimv_refresher_started = 0
let g:slimv_terminate = 0
...
"                let result = system( g:slimv_client . ' -f ' . tmp )
                ...
                execute "python client_file( \"" . tmp . "\", \"" . outfile . "\" )"
                if g:slimv_server_started == 0
                    "let result = system( g:slimv_client . ' -c' )
                    execute "python start_server()"
                    "let result = system( g:slimv_client . ' -f ' . tmp . ' -o ' . outfile )
                    execute "python client_file( \"" . tmp . "\", \"" . outfile . "\" )"
                endif
            else
...
class refresher( Thread ):
    """Thread to refresh REPL output in Vim buffer.
    """
...
def start_server():
    """Spawn server.
    """
#    run_cmd = vim.eval( "g:slimv_client" )
    python_path = vim.eval( "g:slimv_python" )
    slimv_path = vim.eval( "g:slimv_path" )
    lisp_path = vim.eval( "g:slimv_lisp" )
    port = int( vim.eval( "g:slimv_port" ) )

    if True: #run_cmd == '':
        # Complex run command not given, build it from the information available
        if mswindows:
            cmd = [python_path, slimv_path, '-p', str(port), '-l', lisp_path, '-s', 'Slimv.REPL' ]
        else:
            cmd = ['xterm', '-T', 'Slimv', '-e', python_path, slimv_path, '-p', str(port), '-l', lisp_path, '-s', 'Slimv.REPL']
    else:
...
def connect_server():
    ...
    try:
        ...
        s = None
    rs = int( vim.eval( "g:slimv_refresher_started" ) )
    if not rs:
        refresher().start()
        vim.command("let g:slimv_refresher_started = 1")
    return s
...
def client_file( input_filename, output_filename ):
    ...
    s = connect_server()
    if s is None:
        vim.command("let g:slimv_server_started = 0")
        return
    vim.command("let g:slimv_server_started = 1")
