...
if !exists( 'g:slimv_chs_cljapi' )
    let g:slimv_chs_cljapi = [
...
if !exists( 'g:slimv_chs_javadoc' )
    let g:slimv_chs_javadoc = [
...
function! b:HyperspecLookup( word, exact )
    let symbol = ['', '']
    let symbol = SlimvFindSymbol( a:word, a:exact, g:slimv_chs_cljapi,        g:slimv_chs_root,     symbol )
    let symbol = SlimvFindSymbol( a:word, a:exact, g:slimv_chs_javadoc,       g:slimv_javadoc_root, symbol )
    if exists( 'g:slimv_chs_user_db' )
	" Give a choice for the user to extend the symbol database
        if exists( 'g:slimv_chs_user_root' )
            let user_root = g:slimv_chs_user_root
        else
            let user_root = ''
        endif
        let symbol = SlimvFindSymbol( a:word, a:exact, g:slimv_chs_user_db, user_root, symbol )
    endif
    return symbol
endfunction
